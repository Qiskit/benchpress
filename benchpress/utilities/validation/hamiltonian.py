...
def check_hamiltonian_equiv(spop):
    ...
    qk_qct = generate_preset_pass_manager(optimization_level=2, initial_layout=list(range(4)),
                                          backend=qk_backend).run(qk_qc)
    ...
    assert Operator.from_circuit(qk_qct).equiv(bk_qct, atol=1e-6), "Qiskit/Bqskit Hamiltonians not equal"
    # Note that tket and qiskit have different qubit endianess
    rlayout = Layout({q: i for i, q in enumerate(qk_qct.qubits[::-1])})
    assert Operator.from_circuit(qk_qct, layout=rlayout).equiv(tk_qc), "Qiskit/Tket Hamiltonians not equal"


pstr = ['IIII', 'XXYY', 'XYYX', 'YXXY', 'YYXX', 'ZIII', 'ZZII', 'ZIZI', 'ZIIZ', 'IZII', 'IZZI',
        'IZIZ', 'IIZI', 'IIZZ', 'IIIZ']
coeffs = [-1.47217837e+01 + 0.j, -4.83543568e-03 + 0.j, 4.83543568e-03 + 0.j, 4.83543568e-03 + 0.j,
          -4.83543568e-03 + 0.j, 8.17478553e-02 + 0.j, 5.86844977e-02 + 0.j, 3.11424089e-02 + 0.j,
          3.59778446e-02 + 0.j, 8.17478553e-02 + 0.j, 3.59778446e-02 + 0.j, 3.11424089e-02 + 0.j,
          2.83688863e-02 + 0.j, 3.45987447e-02 + 0.j, 2.83688863e-02 + 0.j]
